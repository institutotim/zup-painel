...
angular
  ...
  .controller('AdvancedFiltersAuthorModalController', function($scope, $modalInstance, activeAdvancedFilters, UsersService) {
    ...
    $scope.usersAutocomplete = {
      options: {
        ...
        source: function( request, uiResponse ) {
          ...
          categoriesPromise.then(function(users) {
            uiResponse(_.map(users, function( user ) {
              return {
                label: user.name,
                value: user.name,
